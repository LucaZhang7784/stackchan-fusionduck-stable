...
这是一个在既有 Fusion 验证基础上完成的结构化升级。本版本的主要提升是：网络生命周期从板级初始化中分离，传感器、电源、运动、确认弹窗和报文解析拥有清晰边界；构建支持 MicroDuck 独立轨与生产固件哈希固化；MQTT 心跳、ACK、队列核销和故障诊断均可通过真实日志复核。相较于原先集中式实现，代码更易审计、故障定位路径更短，且保留了可回退的工程基线。
## 架构
