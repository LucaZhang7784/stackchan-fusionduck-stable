...
$out = "$fwroot\firmware\post-fw-v1.0.0-led"
...
if ($LASTEXITCODE -ne 0) { Write-Host "BUILD FAILED (rc=$LASTEXITCODE) - see $log"; exit $LASTEXITCODE }

Copy-Item "$project\build\bootloader\bootloader.bin" $out -Force
Copy-Item "$project\build\partition_table\partition-table.bin" $out -Force
Copy-Item "$project\build\ota_data_initial.bin" $out -Force
Copy-Item "$project\build\StackChan-XiaoZhi.bin" (Join-Path $out "xiaozhi.bin") -Force
Copy-Item "$project\build\srmodels\srmodels.bin" $out -Force -ErrorAction SilentlyContinue
Copy-Item "$project\build\generated_assets.bin" $out -Force -ErrorAction SilentlyContinue
